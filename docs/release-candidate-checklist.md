...
Coverage artifact seam-intent evidence must run in CI before manual smoke starts.
CI installs the Rust coverage helper with
...
must pass before CI uploads `release-artifacts/coverage`. Treat this as named
